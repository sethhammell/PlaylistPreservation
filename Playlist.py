...
def sanitizeFilename(text):
    invalid_chars = '<>:"/\\|?*'
    sanitized = "".join(char for char in text if char not in invalid_chars)
...
def scrapePrivatedOrRemovedSongs(privatedOrRemovedUrls, name):
    urlPrefix = (
        "https://web.archive.org/web/20180203081816/https://www.youtube.com/watch?v="
    )
    youtubePrefix = "www.youtube.com/watch?v="
    ...
    print(
        "Results of Scraping "
        + str(len(privatedOrRemovedUrls))
        + " Privated or Removed Songs from "
        + name
        + ":"
    )
    ...
    def filterRemovedVideos(self, items):
        ...
        for i in items:
            ...
            if i["status"]["privacyStatus"] == "public":
                self.videos.append((newTitle, i["snippet"]["resourceId"]["videoId"]))
    ...
    def getAllPlaylistVideosOld(self):
        ...
        while text.find(newUrl) != -1:
            ...
            while mainTitle == "":
                ...
                for i in range(
                    text.find("<title>") + len("<title>"),
                    text.find(" - YouTube</title>"),
                ):
            ...
            unplayableIndices = [
                i + len(unplayableSearchText)
                for i in range(len(text))
                if text.startswith(unplayableSearchText, i)
            ]
            ...
            urlIndices = [
                i + len(urlSearchText)
                for i in range(len(text))
                if text.startswith(urlSearchText, i)
            ]
