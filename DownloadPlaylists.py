...
def downloadPlaylists(playlists_current):
    ...
    for playlist in playlists_current:
        playlist_path = f'playlists\\{playlist.name}'
        ...
        for video in playlist.videos:
            already_downloaded = any(
                video[1] in s for s in os.listdir(download_folder))

            if not already_downloaded:
                with YoutubeDL({'outtmpl': os.path.join(download_folder, f'{video[0]} - {video[1]}' + '.%(ext)s'), 'format': 'bestvideo+bestaudio/best'}) as ydl:
                    try:
                        ydl.download(youtube_prefix + video[1])
                    except Exception as e:
                        print(
                            f'Error downloading video {video[0]} - {video[1]}')
